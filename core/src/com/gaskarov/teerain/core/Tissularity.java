...
import static com.gaskarov.teerain.resource.Settings.AI_STEPS_PER_TICK;
import static com.gaskarov.teerain.resource.Settings.CELL_STEPS_PER_TICK;
...
import static com.gaskarov.teerain.resource.Settings.DEPTH_FACTORS;
import static com.gaskarov.teerain.resource.Settings.LIGHT_STEPS_PER_TICK;
import static com.gaskarov.teerain.resource.Settings.MAX_CENTER_OFFSET;
import static com.gaskarov.teerain.resource.Settings.MAX_CHUNK_PUSH_PER_TICK;
...
import static com.gaskarov.teerain.resource.Settings.VISITOR_EXTRA_BORDER_SIZE;
import static com.gaskarov.teerain.resource.Settings.VISITOR_SOFT_BORDER_SIZE;
...
public abstract class Tissularity {
	...
	private DynamicCellularity mRayCastBlockCellularity;
	...
	protected LinkedHashTable mPlayers;
	...
	public float getRayCastDistance2() {
		return mRayCastBlockDistance2;
	}

	public DynamicCellularity getRayCastBlockCellularity() {
	...
	protected abstract Runnable chunkLoader(int pX, int pY,
			ChunkHolder pChunkHolder);

	protected abstract Runnable chunkUnloader(int pX, int pY,
			ChunkHolder pChunkHolder);
	...
	public void detach() {
		while (mPlayers.size() > 0) {
			KeyValuePair pair = (KeyValuePair) mPlayers.back();
			removePlayer(((IntVector1) pair.mA).x, (Player) pair.mB);
		}
		while (mVisitors.size() > 0) {
			KeyValuePair p = (KeyValuePair) mVisitors.front();
			Pair pair = (Pair) p.mB;
			IntVector3 tmp = (IntVector3) pair.mB;
			((VisitorOrganoid) p.mA).setIsEnabled((Cellularity) pair.mA, tmp.x,
					tmp.y, tmp.z, false);
		}
		waitChunks();
	...
	public void tick() {
		...
		for (List.Node i = mPlayers.begin(); i != mPlayers.end(); i = mPlayers
				.next(i))
			((Player) ((KeyValuePair) mPlayers.val(i)).mB).tick();
		...
		cellUpdate = MathUtils.divCeil(cellUpdate,
				Math.max(mCellStepsLeft - 1, 1));
		aiUpdate = MathUtils.divCeil(aiUpdate, Math.max(mAIStepsLeft - 1, 1));
		lightUpdate = MathUtils.divCeil(lightUpdate,
				Math.max(mLightStepsLeft - 1, 1));
		...
		for (int i = 0; mDelayedChunks.size() > 0
				&& i < MAX_CHUNK_PUSH_PER_TICK; ++i)
			pushOneDelayedChunk();
		TimeMeasure.sM7.end();
		...
		renderTick(mOffsetX, mOffsetY, mCameraX, mCameraY, getTileRender(),
				camera.viewportWidth / getTileRender(), camera.viewportHeight
						/ getTileRender());
		TimeMeasure.sM11.end();
	}

	protected void renderTick(int pOffsetX, int pOffsetY, float pCameraX,
		...
		for (int chunkRow = chunkRow1; chunkRow <= chunkRow2; ++chunkRow)
			for (int chunkCol = chunkCol1; chunkCol <= chunkCol2; ++chunkCol) {
				...
				if (chunk != null) {
					int localCol1 = Math.max(CHUNK_REGIONS_LEFT, col1
							- (chunkCol << CHUNK_REGIONS_SIZE_LOG));
					int localCol2 = Math.min(CHUNK_REGIONS_RIGHT, col2
							- (chunkCol << CHUNK_REGIONS_SIZE_LOG));
					int localRow1 = Math.max(CHUNK_REGIONS_BOTTOM, row1
							- (chunkRow << CHUNK_REGIONS_SIZE_LOG));
					int localRow2 = Math.min(CHUNK_REGIONS_TOP, row2
							- (chunkRow << CHUNK_REGIONS_SIZE_LOG));
					for (int localRow = localRow1; localRow <= localRow2; ++localRow)
						for (int localCol = localCol1; localCol <= localCol2; ++localCol) {
							LinkedHashTable dynamics = chunk.getDynamics()[localCol
									| (localRow << CHUNK_REGIONS_SIZE_LOG)];
							for (List.Node i = dynamics.begin(); i != dynamics
									.end(); i = dynamics.next(i)) {
								DynamicCellularity cellularity = (DynamicCellularity) dynamics
										.val(i);
								cellularity.render(renderBuffers, pOffsetX,
										pOffsetY, pCameraX, pCameraY,
										pCellSize, chunkCol, chunkRow, pWidth,
										pHeight);
							}
						}
				}
		...
		for (int chunkRow = chunkRow1; chunkRow <= chunkRow2; ++chunkRow)
			for (int chunkCol = chunkCol1; chunkCol <= chunkCol2; ++chunkCol) {
				ChunkCellularity chunk = getChunk(chunkCol, chunkRow);
				if (chunk != null) {
					chunk.render(renderBuffers, pOffsetX, pOffsetY, pCameraX,
							pCameraY, pCellSize, chunkCol, chunkRow, pWidth,
							pHeight);
				}
			}
	...
	protected void init() {
		...
		mPlayers = LinkedHashTable.obtain();
	}
	...
	protected void dispose() {
		...
		mControllables = null;
		LinkedHashTable.recycle(mPlayers);
		mPlayers = null;
	}
	...
	public Player getPlayer(int pId) {
		KeyValuePair pair = (KeyValuePair) mPlayers.get(mVec1.set(pId));
		return pair != null ? (Player) pair.mB : null;
	}

	public void setPlayer(int pId, Player pPlayer) {
		mPlayers.set(KeyValuePair.obtain(IntVector1.obtain(pId), pPlayer));
		pPlayer.attach(this);
	}

	public void removePlayer(int pId, Player pPlayer) {
		pPlayer.detach();
		KeyValuePair pair = (KeyValuePair) mPlayers.remove(mVec1.set(pId));
	...
	public void pushVisitor(VisitorOrganoid pVisitor, Cellularity pCellularity,
			int pX, int pY, int pZ) {
		mVisitors.set(KeyValuePair.obtain(pVisitor,
				Pair.obtain(pCellularity, IntVector3.obtain(pX, pY, pZ))));
	}
	...
	public void moveVisitor(int pOldChunkX, int pOldChunkY, int pOldWidth,
			int pOldHeight, int pChunkX, int pChunkY, int pWidth, int pHeight) {
		if (pOldChunkX == pChunkX && pOldChunkY == pChunkY
				&& pOldWidth == pWidth && pOldHeight == pHeight)
			return;
	...
	public void addVisitor(int pChunkX, int pChunkY, int pWidth, int pHeight) {
		...
		for (int i = softChunkBottom; i <= softChunkTop; ++i)
			for (int j = softChunkLeft; j <= softChunkRight; ++j) {
				...
				if (val == null) {
					mChunkVisitors.set(KeyValuePair.obtain(mVec2.cpy(),
							IntVector1.obtain(1)));
				} else {
					++((IntVector1) val.mB).x;
				}

				if (chunkBottom <= i && i <= chunkTop && chunkLeft <= j
						&& j <= chunkRight) {
	...
	private void loadChunk(int pX, int pY) {
		...
		if (pair == null)
			mChunks.set(pair = KeyValuePair.obtain(mVec2.cpy(),
					ChunkHolder.obtain(this, pX, pY)));
	...
	public int getAI(int pX, int pY, int pZ) {
		ChunkCellularity chunk = getChunk(pX >> CHUNK_SIZE_LOG,
				pY >> CHUNK_SIZE_LOG);
		return chunk == null ? 0 : chunk.getAI(pX & CHUNK_SIZE_MASK, pY
				& CHUNK_SIZE_MASK, pZ);
	}
	...
	public boolean isSolid(int pX, int pY, int pZ) {
		...
		return chunk == null
				|| CellsAction.isSolid(chunk,
						chunk.getCell(localX, localY, pZ), localX, localY, pZ);
	}
	...
	public boolean isBlocking(int pX, int pY, int pZ) {
		...
		final int localY = pY & CHUNK_SIZE_MASK;
		return chunk == null
				|| CellsAction.isBlocking(chunk,
						chunk.getCell(localX, localY, pZ), localX, localY, pZ);
	}

	private void raycastCellDynamic(int pZ, float pStartX, float pStartY,
			float pEndX, float pEndY, DynamicCellularity pCellularity) {

		int startX = MathUtils.floor(pStartX);
		int startY = MathUtils.floor(pStartY);

		if (CellsAction.isBlocking(pCellularity,
				pCellularity.getCell(startX, startY, pZ), startX, startY, pZ)) {
		...
		if (Math.abs(dx) >= FLOAT_EPS)
			for (int posX = startX; posX != endX; posX += vx) {
				...
				if (CellsAction.isBlocking(pCellularity,
						pCellularity.getCell(nextPosX, nextPosY, pZ), nextPosX,
						nextPosY, pZ)) {
					float x1 = x - pStartX;
					float y1 = y - pStartY;
	...
	public void raycastCell(int pZ, float pStartX, float pStartY, float pEndX,
			float pEndY, DynamicCellularity pIgnore) {
		...
		for (int i = chunkBottom; i <= chunkTop; ++i)
			for (int j = chunkLeft; j <= chunkRight; ++j) {
				...
				if (chunk != null) {
					...
					int regionLeft = Math.max(CHUNK_REGIONS_LEFT, left
							- (j << CHUNK_REGIONS_SIZE_LOG));
					int regionBottom = Math.max(CHUNK_REGIONS_BOTTOM, bottom
							- (i << CHUNK_REGIONS_SIZE_LOG));
					int regionRight = Math.min(CHUNK_REGIONS_RIGHT, right
							- (j << CHUNK_REGIONS_SIZE_LOG));
					int regionTop = Math.min(CHUNK_REGIONS_TOP, top
							- (i << CHUNK_REGIONS_SIZE_LOG));
					for (int p = regionBottom; p <= regionTop; ++p)
						for (int q = regionLeft; q <= regionRight; ++q) {
							int m = q | (p << CHUNK_REGIONS_SIZE_LOG);
							for (List.Node k = dynamics[m].begin(); k != dynamics[m]
									.end(); k = dynamics[m].next(k)) {
								DynamicCellularity dynamic = (DynamicCellularity) dynamics[m]
										.val(k);
								if (dynamic != pIgnore) {
									MetaBody body = dynamic.getBody();
									float x1 = pStartX - body.getPositionX()
											- body.getOffsetX();
									float y1 = pStartY - body.getPositionY()
											- body.getOffsetY();
									float x2 = pEndX - body.getPositionX()
											- body.getOffsetX();
									float y2 = pEndY - body.getPositionY()
											- body.getOffsetY();
									float c = body.getCos();
									float s = -body.getSin();
									float dynamicStartX = c * x1 - s * y1
											+ MAX_DROP_HSIZE;
									float dynamicStartY = s * x1 + c * y1
											+ MAX_DROP_HSIZE;
									float dynamicEndX = c * x2 - s * y2
											+ MAX_DROP_HSIZE;
									float dynamicEndY = s * x2 + c * y2
											+ MAX_DROP_HSIZE;
									raycastCellDynamic(pZ, dynamicStartX,
											dynamicStartY, dynamicEndX,
											dynamicEndY, dynamic);
								}
							}
						}
	...
	public int raycastDepth(int pZ, float pX, float pY,
			DynamicCellularity pIgnore) {
		int x = MathUtils.floor(pX);
		int y = MathUtils.floor(pY);
		int left = (x + mOffsetX >> CHUNK_REGION_SIZE_LOG) - 1;
		int bottom = (y + mOffsetY >> CHUNK_REGION_SIZE_LOG) - 1;
		int right = (x + mOffsetX >> CHUNK_REGION_SIZE_LOG) + 1;
		int top = (y + mOffsetY >> CHUNK_REGION_SIZE_LOG) + 1;
		int chunkLeft = left >> CHUNK_REGIONS_SIZE_LOG;
		int chunkBottom = bottom >> CHUNK_REGIONS_SIZE_LOG;
		int chunkRight = right >> CHUNK_REGIONS_SIZE_LOG;
		int chunkTop = top >> CHUNK_REGIONS_SIZE_LOG;
		for (int z = pZ; z < Settings.CHUNK_DEPTH_SKY; ++z) {
			for (int i = chunkBottom; i <= chunkTop; ++i)
				for (int j = chunkLeft; j <= chunkRight; ++j) {
					ChunkCellularity chunk = getChunk(j, i);
					if (chunk != null) {
						...
						int regionLeft = Math.max(CHUNK_REGIONS_LEFT, left
								- (j << CHUNK_REGIONS_SIZE_LOG));
						int regionBottom = Math.max(CHUNK_REGIONS_BOTTOM,
								bottom - (i << CHUNK_REGIONS_SIZE_LOG));
						int regionRight = Math.min(CHUNK_REGIONS_RIGHT, right
								- (j << CHUNK_REGIONS_SIZE_LOG));
						int regionTop = Math.min(CHUNK_REGIONS_TOP, top
								- (i << CHUNK_REGIONS_SIZE_LOG));
						for (int p = regionBottom; p <= regionTop; ++p)
							for (int q = regionLeft; q <= regionRight; ++q) {
								...
								for (List.Node k = dynamics[m].begin(); k != dynamics[m]
									...
									if (dynamic != pIgnore) {
										MetaBody body = dynamic.getBody();
										float x1 = pX - body.getPositionX()
												- body.getOffsetX();
										float y1 = pY - body.getPositionY()
												- body.getOffsetY();
										float c = body.getCos();
										float s = -body.getSin();
										int dynamicX = MathUtils.floor(c * x1
												- s * y1 + MAX_DROP_HSIZE);
										int dynamicY = MathUtils.floor(s * x1
												+ c * y1 + MAX_DROP_HSIZE);
										if (CellsAction.isBlocking(dynamic,
												dynamic.getCell(dynamicX,
														dynamicY, z), dynamicX,
												dynamicY, z)) {
											mRayCastBlockCellX = dynamicX;
											mRayCastBlockCellY = dynamicY;
											mRayCastBlockCellularity = dynamic;
											return z;
										}
									}
								}
			...
			if (isBlocking(x + mOffsetX, y + mOffsetY, z)) {
				mRayCastBlockCellX = x + mOffsetX;
				mRayCastBlockCellY = y + mOffsetY;
				mRayCastBlockCellularity = null;
				return z;
			}
		}
		return Settings.CHUNK_DEPTH_SKY;
	}
	...
	protected final static class ChunkHolder {
		...
		private static ChunkHolder obtainPure() {
			if (GlobalConstants.POOL)
				synchronized (ChunkHolder.class) {
					return sPool.size() == 0 ? new ChunkHolder()
							: (ChunkHolder) sPool.pop();
				}
		...
		public static ChunkHolder obtain(Tissularity pTissularity, int pX,
				int pY) {
		...
		public void finish() {
			if (mState == STATE_LOADING) {
				...
				for (int i = 0; i < ArrayConstants.MOVE_AROUND_SIZE; ++i) {
					...
					ChunkCellularity chunk = mTissularity.getChunk(mX + vx, mY
							+ vy);
					if (chunk != null) {
