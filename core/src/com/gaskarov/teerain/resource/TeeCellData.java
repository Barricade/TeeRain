...
public class TeeCellData extends CellData {
	...
	public TeeCellData cpy() {
		return obtain(mControlOrganoid.getId(), mVisitorOrganoid.getIsEnabled());
	}
	...
	private static TeeCellData obtainPure() {
		if (GlobalConstants.POOL)
			synchronized (TeeCellData.class) {
				return sPool.size() == 0 ? new TeeCellData()
						: (TeeCellData) sPool.pop();
			}
	...
	public static TeeCellData obtain(int pId, boolean pMob) {
		TeeCellData obj = obtainPure();
		obj.mControlOrganoid = ControlOrganoid.obtain(pId);
		obj.mVisitorOrganoid = VisitorOrganoid.obtain(!pMob);
		return obj;
	}
	...
	public int getId() {
		return mControlOrganoid.getId();
	}

	public float getDropDelayLeft() {
		return mControlOrganoid.getDropDelayLeft();
	}
	...
	public void render(int pCell, Cellularity pCellularity, int pX, int pY,
		...
		FloatArray renderBuffer = pRenderBuffers[pZ * Settings.LAYERS_PER_DEPTH
		...
		if (mControlOrganoid.isGrounded()) {
			float tmp = MathUtils.mod(backSteps + (float) Math.PI,
					(float) Math.PI * 2);
		...
		GraphicsUtils.renderTexture(pCellularity, pX, pY, pZ, pOffsetX,
				pOffsetY, pTileX, pTileY, pSize, pCos, pSin, renderBuffer,
				renderData[2], renderData[3], Settings.TILE_HALF_W,
				Settings.TILE_QUARTER_H, footBackX, footBackY, footBackW,
				footBackH, (float) Math.cos(footBackR),
				(float) Math.sin(footBackR));

		Player player = pCellularity.getTissularity().getPlayer(
				mControlOrganoid.getId());
		if (player != null)
			CellsAction.renderWeapon(pCellularity,
					player.getItem(player.getUseItem()),
					player.getItemData(player.getUseItem()), pX, pY, pZ,
					pOffsetX, pOffsetY, pTileX, pTileY, pSize, pCos, pSin,
					renderBuffer, mControlOrganoid);

		GraphicsUtils.render(pCell, pCellularity, pX, pY, pZ, pOffsetX,
				pOffsetY, pTileX, pTileY, pSize, pCos, pSin, renderBuffer,
				renderData[0], renderData[1]);
		...
		GraphicsUtils.renderTexture(pCellularity, pX, pY, pZ, pOffsetX,
				pOffsetY, pTileX, pTileY, pSize, pCos, pSin, renderBuffer,
				renderData[4], renderData[5], Settings.TILE_HALF_W,
		...
		GraphicsUtils.renderTexture(pCellularity, pX, pY, pZ, pOffsetX,
				...
				footFrontH, (float) Math.cos(footFrontR),
				(float) Math.sin(footFrontR));
	}
	...
	private static float getFootPositionR(float pDist) {
		...
		if (pDist < Math.PI / 2) {
			...
			if (pDist < Math.PI * 1.75) {
				...
				footR = (float) Math
						.sin((float) (Math.PI / 2 - (pDist - Math.PI * 1.5))) * 0.5f;
			}
		}
